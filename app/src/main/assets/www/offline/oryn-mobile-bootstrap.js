(function(){
'use strict';
const BUILD='ORYN-ANDROID-V10.4.1-SCARA-V04-20260910-1';
...
async function scaraUploadText(cfg,name,txt){const filename=safeScaraFilename(name);const fd=new FormData();fd.append('file',new Blob([txt],{type:'text/plain'}),filename);const r=await scaraRaw(cfg.host,'/api/upload',{method:'POST',body:fd});if(!r.ok){let t='';try{t=await r.text();}catch(_){}throw new Error(t||'SCARA pattern upload failed.');}return filename;}
...
async function waitScaraPattern(cfg){while(!scaraState.stopping){const st=await refreshScaraStatus(cfg);if(!st.playing){if(Number(st.progress||0)>=99.9){scaraState.last_completed_time=0;return true;}if(st.last_error)throw new Error(st.last_error);return true;}await new Promise(r=>setTimeout(r,260));}return false;}
async function startScaraSequence(seq){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');if(scaraWorker||scaraState.running)throw new Error('Another SCARA pattern is already running.');scaraState.stopping=false;scaraState.error='';scaraWorker=(async()=>{try{for(const item of seq){if(scaraState.stopping)break;scaraState.current_file=item.display||item.path;const raw=await patternText(item.path);const txt=item.rotate===false?raw:rotateThrText(raw,readPatternOrientationDegrees());const remote=await scaraUploadText(cfg,item.display||item.path,txt);await scaraJson(cfg.host,'/api/play?file='+encodeURIComponent(remote),{method:'POST'});scaraState.running=true;await waitScaraPattern(cfg);} }catch(e){scaraState.error=String(e&&e.message||e);mobileToast(scaraState.error);}finally{scaraState.running=false;scaraState.paused=false;scaraWorker=null;}})();return true;}
async function scaraFetch(u,init={}){
 ...
 try{
  ...
  if(p==='/send_home')return errResponse('ORYN SCARA uses a physical 152 mm reference. Use the SCARA SET REFERENCE button, then use Center/Perimeter.',409);
  if(p==='/move_to_center'||p==='/move_to_perimeter'){const st=await refreshScaraStatus(cfg);if(!st.referenced)return errResponse('Set the physical SCARA reference first.',409);const rho=p==='/move_to_center'?0:1;await scaraJson(cfg.host,'/api/move_polar?theta_deg='+encodeURIComponent(Number(st.theta_deg||0))+'&rho='+rho,{method:'POST'});return jsonResponse({success:true,scara:true});}
...
function installScaraReferenceButton(){
 if(!scaraConfig()||document.getElementById('oryn-scara-reference-btn'))return;const buttons=[...document.querySelectorAll('button')];const home=buttons.find(b=>/^\s*home\s*$/i.test(String(b.textContent||'')));if(!home||!home.parentNode)return;const b=home.cloneNode(true);b.id='oryn-scara-reference-btn';b.textContent='SCARA SET REFERENCE';b.onclick=async e=>{e.preventDefault();e.stopPropagation();if(!confirm('Place both 76 mm arms straight at the +X perimeter (magnet center 152 mm from main shaft), then set this as Theta 0° / Rho 1.00. Continue?'))return;const cfg=scaraConfig();try{await scaraJson(cfg.host,'/api/set_reference',{method:'POST'});mobileToast('SCARA reference set ✓  Theta 0° / Rho 1.00');}catch(err){mobileToast('Reference failed: '+String(err&&err.message||err));}};home.parentNode.insertBefore(b,home.nextSibling);
}
new MutationObserver(()=>{installNativeButtons();installPatternOrientationControl();installScaraReferenceButton();}).observe(document.documentElement,{childList:true,subtree:true});
