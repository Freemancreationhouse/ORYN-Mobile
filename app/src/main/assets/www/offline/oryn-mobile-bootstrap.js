...
function readPatternOrientationDegrees(){try{return normalizePatternOrientation(localStorage.getItem(DIRECT_PATTERN_ORIENTATION_KEY)||0);}catch(_){return 0;}}
function savePatternOrientationDegrees(value){const n=normalizePatternOrientation(value);try{localStorage.setItem(DIRECT_PATTERN_ORIENTATION_KEY,String(n));}catch(_){}return n;}
...
function wifiSetupTarget(){return activeRemote()?'pi':(directConfig()?'direct':'offline');}
...
new MutationObserver(()=>{installNativeButtons();installPatternOrientationControl();}).observe(document.documentElement,{childList:true,subtree:true});
