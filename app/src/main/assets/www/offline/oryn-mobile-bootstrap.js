...
const SCARA_PATTERN_ORIENTATION_KEY='oryn_scara_pattern_orientation_degrees_v1';
const SCARA_DEFAULT_HOST='oryn-scara.local';
const SCARA_AP_HOST='192.168.4.1';
const SCARA_AUTO_HOME_KEY='oryn_scara_auto_home_v1';
const SCARA_SPEED_KEY='oryn_scara_speed_mm_s_v1';
const SCARA_CAL_KEY='oryn_scara_calibration_check_v1';
const DIRECT_AUTO_HOME_PENDING='oryn_direct_auto_home_pending_v1';
const DIRECT_PATTERN_ORIENTATION_KEY='oryn_direct_pattern_orientation_degrees_v1';
...
function readScaraSavedRaw(){try{const x=JSON.parse(localStorage.getItem('oryn_scara_last')||'{}');return x&&typeof x==='object'?x:{};}catch(_){return {};}}
function scaraDisplayName(){try{return String(localStorage.getItem('oryn_scara_name')||'ORYN SCARA — ESP32').trim()||'ORYN SCARA — ESP32';}catch(_){return 'ORYN SCARA — ESP32';}}
function validSavedScara(cfg){return !!(cfg&&String(cfg.host||'').trim());}
function makeScaraTable(cfg,isCurrent){return {id:SCARA_ID,name:scaraDisplayName(),url:location.origin,host:String(cfg.host||SCARA_DEFAULT_HOST),scaraHost:String(cfg.host||SCARA_DEFAULT_HOST),scaraEsp32:true,radiusMm:Number(cfg.radius_mm||152),arm1Mm:Number(cfg.arm1_mm||76),arm2Mm:Number(cfg.arm2_mm||76),isOnline:true,isCurrent:!!isCurrent,version:String(cfg.firmware||'V0.4')};}
function scaraTable(){try{const d=JSON.parse(localStorage.getItem('orynmotion_tables')||'{}'),id=localStorage.getItem('orynmotion_active_table');return (d.tables||[]).find(t=>t&&t.id===id&&t.scaraEsp32)||null;}catch(_){return null;}}
function scaraConfig(){const t=scaraTable();if(t)return {host:String(t.scaraHost||t.host||SCARA_DEFAULT_HOST),radius_mm:Number(t.radiusMm||152),arm1_mm:Number(t.arm1Mm||76),arm2_mm:Number(t.arm2Mm||76),firmware:String(t.version||'V0.4')};try{if(localStorage.getItem('orynmotion_active_table')!==SCARA_ID||localStorage.getItem('oryn_scara_enabled')!=='1')return null;const x=readScaraSavedRaw();if(!validSavedScara(x))return null;return {host:String(x.host||SCARA_DEFAULT_HOST),radius_mm:Number(x.radius_mm||152),arm1_mm:Number(x.arm1_mm||76),arm2_mm:Number(x.arm2_mm||76),firmware:String(x.firmware||'V0.4')};}catch(_){return null;}}
function scaraBase(host){let h=String(host||SCARA_DEFAULT_HOST).trim().replace(/^https?:\/\//i,'').replace(/\/+$/,'');return 'http://'+h;}
async function scaraRaw(host,path,init={}){return nativeFetch(scaraBase(host)+path,{cache:'no-store',...init});}
async function scaraJson(host,path,init={}){const r=await scaraRaw(host,path,init);const txt=await r.text();let data={};try{data=txt?JSON.parse(txt):{};}catch(_){data={detail:txt||('HTTP '+r.status)}}if(!r.ok)throw new Error(data.error||data.detail||('HTTP '+r.status));return data;}
async function probeScara(host){const h=String(host||SCARA_DEFAULT_HOST).trim();try{const d=await scaraJson(h,'/api/device');if(String(d.controller||'')!=='oryn_scara')throw new Error('This address is not an ORYN SCARA controller.');return {ok:true,host:h,device:d};}catch(e){return {ok:false,host:h,error:String(e&&e.message||e)};}}
function activateScaraTable(host,device={}){const h=String(host||'').trim();if(!h){mobileToast('SCARA IP/hostname is missing.');return false;}const saved={host:h,radius_mm:Number(device.radius_mm||152),arm1_mm:Number(device.arm1_mm||76),arm2_mm:Number(device.arm2_mm||76),firmware:String(device.firmware||'V0.4')};try{localStorage.setItem('oryn_scara_last',JSON.stringify(saved));localStorage.setItem('oryn_scara_enabled','1');localStorage.setItem('oryn_direct_enabled','0');localStorage.setItem('orynmotion_active_table',SCARA_ID);const raw=JSON.parse(localStorage.getItem('orynmotion_tables')||'{}'),tables=Array.isArray(raw.tables)?raw.tables:[];const remotes=tables.filter(x=>x&&x.id!==OFFLINE_ID&&x.id!==DIRECT_ID&&x.id!==SCARA_ID&&x.url&&new URL(x.url,location.origin).origin!==location.origin).map(x=>({...x,isCurrent:false}));const merged=[makeScaraTable(saved,true),makeOfflineTable(false),...remotes];localStorage.setItem('orynmotion_tables',JSON.stringify({tables:merged,activeTableId:SCARA_ID}));mobileToast('ORYN SCARA connected ✓');setTimeout(()=>location.reload(),250);return true;}catch(e){mobileToast('Could not activate ORYN SCARA: '+e);return false;}}
function ensureOfflineTable(forceActive){
 try{
  ...
  const remotes=old.filter(t=>t&&t.id!==OFFLINE_ID&&t.id!==DIRECT_ID&&t.id!==SCARA_ID&&t.url&&new URL(t.url,location.origin).origin!==location.origin).map(t=>({...t,isCurrent:false}));
  let active=localStorage.getItem('orynmotion_active_table')||raw.activeTableId||OFFLINE_ID;
  const directSaved=readDirectSavedRaw(),scaraSaved=readScaraSavedRaw();
  if(forceActive){active=OFFLINE_ID;localStorage.setItem('oryn_direct_enabled','0');localStorage.setItem('oryn_scara_enabled','0');}
  let directActive=!forceActive&&active===DIRECT_ID&&localStorage.getItem('oryn_direct_enabled')==='1'&&validSavedDirect(directSaved);
  let scaraActive=!forceActive&&active===SCARA_ID&&localStorage.getItem('oryn_scara_enabled')==='1'&&validSavedScara(scaraSaved);
  if(active===DIRECT_ID&&!directActive)active=OFFLINE_ID;
  if(active===SCARA_ID&&!scaraActive)active=OFFLINE_ID;
  if(active!==DIRECT_ID)localStorage.setItem('oryn_direct_enabled','0');
  if(active!==SCARA_ID)localStorage.setItem('oryn_scara_enabled','0');
  if(active!==OFFLINE_ID&&active!==DIRECT_ID&&active!==SCARA_ID&&!remotes.some(t=>t.id===active))active=OFFLINE_ID;
  const tables=directActive?[makeDirectTable(directSaved,true),makeOfflineTable(false),...remotes]:scaraActive?[makeScaraTable(scaraSaved,true),makeOfflineTable(false),...remotes]:[makeOfflineTable(true),...remotes];
 ...
 }catch(_){try{localStorage.setItem('oryn_direct_enabled','0');localStorage.setItem('oryn_scara_enabled','0');localStorage.setItem('orynmotion_active_table',OFFLINE_ID);localStorage.setItem('orynmotion_tables',JSON.stringify({tables:[makeOfflineTable(true)],activeTableId:OFFLINE_ID}));}catch(__){}}
}
...
if(ORYN_COLD_START){
 try{localStorage.setItem('oryn_direct_enabled','0');localStorage.setItem('oryn_scara_enabled','0');}catch(_){}
...
function activeRemote(){try{const raw=localStorage.getItem('orynmotion_tables'),id=localStorage.getItem('orynmotion_active_table');if(!raw||!id)return null;const d=JSON.parse(raw);const t=(d.tables||[]).find(x=>x.id===id);return t&&t.id!==OFFLINE_ID&&t.id!==DIRECT_ID&&t.id!==SCARA_ID&&!t.isCurrent&&t.url?t:null;}catch(_){return null;}}
function normalizePatternOrientation(value){const n=Number(value);if(!Number.isFinite(n))return 0;return ((n%360)+360)%360;}
function orientationStorageKey(){return scaraConfig()?SCARA_PATTERN_ORIENTATION_KEY:DIRECT_PATTERN_ORIENTATION_KEY;}
function readPatternOrientationDegrees(){try{return normalizePatternOrientation(localStorage.getItem(orientationStorageKey())||0);}catch(_){return 0;}}
function savePatternOrientationDegrees(value){const n=normalizePatternOrientation(value);try{localStorage.setItem(orientationStorageKey(),String(n));}catch(_){}return n;}
...
let __orynWifiConnectMode='direct';
function scaraModal(){return document.getElementById('oryn-scara-modal');}
function scaraModalStatus(text,error=false){const e=document.getElementById('oryn-scara-status');if(!e)return;e.textContent=text||'';e.style.color=error?'#ff9b9b':'#d9e7db';}
function closeScaraModal(){const e=scaraModal();if(e)e.remove();}
async function probeAndActivateScara(host){const h=String(host||'').trim();if(!h)return false;scaraModalStatus('Checking ORYN SCARA at '+h+'…');const p=await probeScara(h);if(!p.ok){scaraModalStatus('SCARA did not answer at '+h+'. '+p.error,true);return false;}scaraModalStatus('ORYN SCARA '+String(p.device.firmware||'')+' found ✓');setTimeout(()=>activateScaraTable(h,p.device),180);return true;}
async function findScaraCurrentNetwork(){const saved=readScaraSavedRaw();const candidates=[];for(const h of [saved.host,SCARA_DEFAULT_HOST])if(h&&!candidates.includes(h))candidates.push(h);scaraModalStatus('Searching current Wi‑Fi / hotspot for ORYN SCARA…');for(const h of candidates){const p=await probeScara(h);if(p.ok){scaraModalStatus('ORYN SCARA found at '+h+' ✓');setTimeout(()=>activateScaraTable(h,p.device),150);return true;}}scaraModalStatus('No ORYN SCARA found automatically. Use its LAN IP or connect to the ORYN-SCARA AP.',true);return false;}
function openScaraConnect(){
 closeScaraModal();const wrap=document.createElement('div');wrap.id='oryn-scara-modal';wrap.style.cssText='position:fixed;inset:0;z-index:2147483001;background:rgba(0,0,0,.78);display:flex;align-items:center;justify-content:center;padding:16px;font-family:system-ui,sans-serif';
 const card=document.createElement('div');card.style.cssText='width:min(650px,100%);max-height:90vh;overflow:auto;background:#0f0f0f;border:1px solid #333;border-radius:20px;padding:18px;box-shadow:0 24px 80px #000;color:#eee';
 const saved=readScaraSavedRaw();card.innerHTML=`<div style="display:flex;justify-content:space-between;gap:12px"><div><div style="font-size:20px;font-weight:800">ORYN SCARA — ESP32</div><div style="font-size:12px;color:#aaa;margin-top:3px">76 + 76 mm · 152 mm · Firmware V0.4</div></div><button id="oryn-scara-close" style="border:0;background:#282828;color:#fff;border-radius:9px;padding:8px 11px">✕</button></div><div style="display:grid;gap:9px;margin-top:15px"><button id="oryn-scara-find" class="oryn-sw-btn">Find SCARA on Current Wi‑Fi / Hotspot</button><button id="oryn-scara-ap" class="oryn-sw-btn">Connect to ORYN-SCARA AP</button></div><div id="oryn-scara-status" style="min-height:22px;margin:12px 0;font-size:13px;color:#d9e7db">Use automatic discovery first. AP fallback remains 192.168.4.1.</div><details style="margin-top:10px"><summary style="cursor:pointer;font-weight:700">Manual IP fallback</summary><div style="display:flex;gap:8px;margin-top:8px"><input id="oryn-scara-host" value="${escapeHtml(saved.host||SCARA_DEFAULT_HOST)}" style="flex:1;padding:10px;border-radius:9px;border:1px solid #333;background:#181818;color:#fff"><button id="oryn-scara-go" class="oryn-sw-btn">Connect</button></div></details><div style="font-size:12px;color:#aaa;margin-top:14px;line-height:1.5">For AP fallback the app connects to <b>ORYN-SCARA</b> using the prototype password and then talks to <b>192.168.4.1</b>. Home Wi‑Fi and phone-hotspot connections use the IP assigned by that network or <b>oryn-scara.local</b>.</div>`;
 const style=document.createElement('style');style.textContent='.oryn-sw-btn{border:1px solid #3b3b3b;border-radius:10px;padding:11px 12px;background:#202020;color:#fff;font-weight:700}';card.appendChild(style);wrap.appendChild(card);document.body.appendChild(wrap);
 document.getElementById('oryn-scara-close').onclick=closeScaraModal;wrap.addEventListener('click',e=>{if(e.target===wrap)closeScaraModal();});
 document.getElementById('oryn-scara-find').onclick=()=>findScaraCurrentNetwork();
 document.getElementById('oryn-scara-ap').onclick=()=>{__orynWifiConnectMode='scara';scaraModalStatus('Requesting ORYN-SCARA Wi‑Fi… approve Android connection if asked.');try{window.OrynAndroid.connectDirectWifi('ORYN-SCARA','oryn12345');}catch(e){scaraModalStatus(String(e),true);}};
 document.getElementById('oryn-scara-go').onclick=()=>{const h=document.getElementById('oryn-scara-host').value.trim();if(h)probeAndActivateScara(h);};
}
...
function openSmartWifi(){
 __orynWifiConnectMode='direct';closeSmartWifi();const st=directWifiState();
 ...
 <details style="margin-top:16px;border-top:1px solid #292929;padding-top:12px"><summary style="cursor:pointer;font-weight:700">Wi‑Fi network setup (change anytime)</summary><div style="font-size:12px;color:#aaa;margin:8px 0">FluidNC supports 2.4 GHz Wi-Fi. Keep this phone connected to the FluidNC AP, detect networks from the ESP32, choose the new router/hotspot, save, then power-cycle and use Find ESP32 on Current Network.</div><div style="font-size:12px;color:#e2c86f;margin:8px 0;padding:9px;border:1px solid #514725;border-radius:9px;background:#1b180e"><strong>Using this phone's hotspot?</strong> It may not appear while this phone is connected to FluidNC. Enter its SSID manually, save it, disconnect from FluidNC Wi-Fi, enable the hotspot in 2.4 GHz mode, and only then power-cycle the ESP32.</div><button id="oryn-home-detect" class="oryn-sw-btn" style="width:100%;margin:5px 0">Detect 2.4 GHz Networks from ESP32</button><select id="oryn-home-networks" style="display:none;width:100%;box-sizing:border-box;margin:5px 0;padding:10px;border-radius:9px;border:1px solid #333;background:#181818;color:#fff"></select><input id="oryn-home-ssid" placeholder="Wi‑Fi network name (SSID)" style="width:100%;box-sizing:border-box;margin:5px 0;padding:10px;border-radius:9px;border:1px solid #333;background:#181818;color:#fff"><input id="oryn-home-pass" type="password" placeholder="Wi‑Fi network password" style="width:100%;box-sizing:border-box;margin:5px 0;padding:10px;border-radius:9px;border:1px solid #333;background:#181818;color:#fff"><button id="oryn-home-save" class="oryn-sw-btn" style="width:100%;margin-top:6px">Verify & Save Wi‑Fi to ESP32</button></details>
 ...
 const fluidWifiHost=()=>{const ws=directWifiState(),cfg=directConfig(),saved=readDirectSavedRaw();return (ws&&ws.connected)?'192.168.0.1':String((cfg&&cfg.host)||saved.host||'192.168.0.1');};
 document.getElementById('oryn-home-detect').onclick=()=>{const host=fluidWifiHost();smartWifiStatus('Asking FluidNC to scan nearby 2.4 GHz networks…');const r=directBridgeJson('scanFluidNcWifi',host);const select=document.getElementById('oryn-home-networks');if(!r||!r.success){select.style.display='none';smartWifiStatus((r&&r.detail)||'FluidNC Wi-Fi scan failed.',true);return;}const nets=Array.isArray(r.networks)?r.networks:[];select.innerHTML='<option value="">Choose a detected network…</option>'+nets.map(n=>`<option value="${escapeHtml(n.ssid)}">${escapeHtml(n.ssid)} · ${Number(n.signal||0)}%${n.secure?' · secured':' · open'}</option>`).join('');select.style.display=nets.length?'block':'none';select.onchange=()=>{if(select.value)document.getElementById('oryn-home-ssid').value=select.value;};smartWifiStatus(r.message||'ESP32 Wi-Fi scan finished.',!nets.length);};
 document.getElementById('oryn-home-save').onclick=()=>{const ssid=document.getElementById('oryn-home-ssid').value.trim(),pass=document.getElementById('oryn-home-pass').value;if(!ssid){smartWifiStatus('Detect or enter the 2.4 GHz Wi‑Fi network name.',true);return;}if(pass&&pass.length<8){smartWifiStatus('Wi‑Fi password must be at least 8 characters.',true);return;}const host=fluidWifiHost();smartWifiStatus('Writing and verifying Wi‑Fi settings on FluidNC at '+host+'…');const r=directBridgeJson('configureFluidNcHomeWifi',host,ssid,pass);if(r&&r.success)smartWifiStatus('Verified and saved for '+ssid+'. Router: power-cycle ESP32. This phone hotspot: first disconnect FluidNC Wi-Fi, enable the hotspot at 2.4 GHz, then power-cycle ESP32. After it joins, use Find ESP32 on Current Network.');else smartWifiStatus((r&&r.detail)||'FluidNC did not verify the new Wi‑Fi settings.',true);};
...
window.__orynNativeWifiConnection=function(state){if(__orynWifiConnectMode==='scara'){if(!scaraModal())openScaraConnect();const bad=state&&['error','unavailable','lost','unsupported'].includes(state.status);scaraModalStatus((state&&state.message)||'Wi‑Fi status changed.',bad);if(state&&state.status==='connected'){scaraModalStatus('ORYN-SCARA Wi‑Fi connected ✓ Detecting SCARA controller…');setTimeout(()=>probeAndActivateScara(SCARA_AP_HOST),650);}return;}if(!smartWifiModal())openSmartWifi();const bad=state&&['error','unavailable','lost','unsupported'].includes(state.status);if(state&&state.ssid)setWifiRowState(state.ssid,state.status);const msg=(state&&state.message)||'Wi‑Fi status changed.';smartWifiStatus(msg,bad);if(state&&state.status==='connected'){smartWifiStatus((state.internet_available?'Internet available. ':'')+'FluidNC Wi‑Fi connected ✓  Detecting controller…');setTimeout(()=>probeAndActivateDirect('192.168.0.1'),650);}};
...
function installPatternOrientationControl(){
 ...
 if(!directConfig()&&!scaraConfig()){if(old)old.remove();return;}
 ...
 const show=n=>{const v=savePatternOrientationDegrees(n);value.textContent=v.toFixed(Number.isInteger(v)?0:1)+'°';input.value=String(v);for(const b of box.querySelectorAll('[data-oryn-angle]'))b.style.outline=Number(b.dataset.orynAngle)===v?'2px solid #f1c75b':'none';};
 for(const b of box.querySelectorAll('button'))styleButton(b);
 for(const b of box.querySelectorAll('[data-oryn-angle]'))b.onclick=()=>show(Number(b.dataset.orynAngle));
 box.querySelector('#oryn-orientation-apply').onclick=()=>show(input.value);
 input.onkeydown=e=>{if(e.key==='Enter')show(input.value);};
 clearBlock.parentNode.insertBefore(box,clearBlock);show(readPatternOrientationDegrees());
}

// Route Wi-Fi setup by the active table type. A selected remote ORYN/Pi must
// retain the real Pi /wifi-setup page and its remote API. Direct ESP32 and
// Offline modes use Android Smart Wi-Fi so the local Pi screen cannot blank.
function wifiSetupTarget(){return activeRemote()?'pi':(scaraConfig()?'scara':(directConfig()?'direct':'offline'));}
function openWifiSetupForActiveTable(navigate){
 const target=wifiSetupTarget();
 if(target==='pi'){
  if(typeof navigate==='function')navigate('/wifi-setup');
  return target;
 }
 if(target==='scara')openScaraConnect();else openSmartWifi();
 return target;
}
...
const scaraState={running:false,paused:false,stopping:false,current_file:null,current:0,total:0,percentage:0,theta:0,rho:1,error:'',last_completed_time:0,speed:60,total_time:0,elapsed_time:0,remaining_time:0,time_cumulative:[]};
...
function scaraSavedSpeed(){try{const v=Number(localStorage.getItem(SCARA_SPEED_KEY)||60);return Math.max(10,Math.min(100,Number.isFinite(v)?v:60));}catch(_){return 60;}}
function saveScaraSpeed(v){const n=Math.max(10,Math.min(100,Number(v)||60));try{localStorage.setItem(SCARA_SPEED_KEY,String(n));}catch(_){}scaraState.speed=n;return n;}
async function applyScaraSpeed(v){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');const n=saveScaraSpeed(v);const st=await scaraJson(cfg.host,'/api/speed?mm_s='+encodeURIComponent(n),{method:'POST'});scaraState.speed=Number(st.speed_mm_s||n);return scaraState.speed;}
function readScaraCalibrationCheck(){try{return JSON.parse(localStorage.getItem(SCARA_CAL_KEY)||'{}')||{};}catch(_){return {};}}
function saveScaraCalibrationCheck(patch){const next={...readScaraCalibrationCheck(),...(patch||{})};try{localStorage.setItem(SCARA_CAL_KEY,JSON.stringify(next));}catch(_){}return next;}
async function scaraRequireReference(cfg){const st=await refreshScaraStatus(cfg);if(!st.referenced)throw new Error('SCARA reference required. Place both 76 mm arms straight at +X / 152 mm and press SET REFERENCE.');return st;}
async function scaraMovePolarDegrees(thetaDeg,rho){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');await scaraRequireReference(cfg);await scaraJson(cfg.host,'/api/move_polar?theta_deg='+encodeURIComponent(Number(thetaDeg||0))+'&rho='+encodeURIComponent(Number(rho)),{method:'POST'});return await refreshScaraStatus(cfg);}
async function scaraSetReferenceAction(){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');await scaraJson(cfg.host,'/api/set_reference',{method:'POST'});saveScaraCalibrationCheck({reference:true,reference_at:new Date().toISOString()});const st=await refreshScaraStatus(cfg);if(scaraAutoHomeEnabled()){await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho=0',{method:'POST'});mobileToast('SCARA reference set ✓  Auto Home → Centre complete.');}else mobileToast('SCARA reference set ✓  Theta 0° / Rho 1.00');return st;}
async function scaraHomeAction(){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');await scaraRequireReference(cfg);await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho=0',{method:'POST'});mobileToast('SCARA Home / Centre complete ✓');return true;}
async function scaraPerimeterAction(){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');await scaraRequireReference(cfg);await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho=1',{method:'POST'});mobileToast('SCARA Perimeter → 152 mm ✓');return true;}
async function scaraThirtyDegreeTest(){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');await scaraRequireReference(cfg);await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho=0.95',{method:'POST'});await scaraJson(cfg.host,'/api/move_polar?theta_deg=30&rho=0.95',{method:'POST'});saveScaraCalibrationCheck({rotation_30:true,rotation_30_at:new Date().toISOString()});mobileToast('30° calibration test complete ✓');return true;}
async function scaraFullCircleTest(){const cfg=scaraConfig();if(!cfg)throw new Error('ORYN SCARA is not active.');await scaraRequireReference(cfg);await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho=0.95',{method:'POST'});for(let d=30;d<=360;d+=30){await scaraJson(cfg.host,'/api/move_polar?theta_deg='+d+'&rho=0.95',{method:'POST'});}saveScaraCalibrationCheck({rotation_360:true,rotation_360_at:new Date().toISOString()});mobileToast('360° full-circle calibration test complete ✓');return true;}
async function scaraRadiusTest(){await scaraPerimeterAction();saveScaraCalibrationCheck({radius_152:true,radius_152_at:new Date().toISOString()});mobileToast('Perimeter calibration position reached — magnet centre should measure 152 mm.');return true;}
async function maybeScaraAutoHome(reason='connect'){if(!scaraConfig()||!scaraAutoHomeEnabled())return false;try{const cfg=scaraConfig(),st=await refreshScaraStatus(cfg);if(!st.referenced){if(reason==='connect')mobileToast('SCARA connected. Set Reference once to enable automatic Home.');return false;}if(st.playing)return false;await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho=0',{method:'POST'});mobileToast('SCARA Auto Home → Centre ✓');return true;}catch(e){console.warn('SCARA auto home skipped',e);return false;}}
let scaraWorker=null;
...
function buildScaraTiming(pts,radiusMm,speedMmS){
 const radius=Math.max(0.1,Number(radiusMm)||152),speed=Math.max(0.1,Number(speedMmS)||60),cum=new Array(pts.length).fill(0);let total=0;
...
async function scaraUploadText(cfg,name,txt){
 ...
 fd.append('file',new Blob([txt],{type:'text/plain'}),filename);
 const r=await scaraRaw(cfg.host,'/api/upload',{method:'POST',body:fd});
 if(!r.ok){let t='';try{t=await r.text();}catch(_){}throw new Error(t||'SCARA pattern upload failed.');}
 // V0.4 upload response is deliberately small, so verify the file actually reached LittleFS.
 const files=await scaraJson(cfg.host,'/api/files');
 const found=Array.isArray(files)&&files.some(f=>String((f&&f.name)||'').split('/').pop()===filename&&Number((f&&f.size)||0)>0);
 if(!found)throw new Error('SCARA pattern upload was not stored on the ESP32.');
 return filename;
}
async function refreshScaraStatus(cfg){try{const st=await scaraJson(cfg.host,'/api/status');scaraState.running=!!st.playing;scaraState.paused=!!st.paused;scaraState.percentage=Number(st.progress||0);scaraState.current=Number(st.played_points||0);if(!(Number(scaraState.total)>0))scaraState.total=Math.max(scaraState.current,1);else scaraState.total=Math.max(scaraState.total,scaraState.current);scaraState.theta=Number(st.theta_deg||0)*Math.PI/180;scaraState.rho=Number(st.rho||0);if(Number(st.speed_mm_s)>0)scaraState.speed=Number(st.speed_mm_s);scaraState.error=String(st.last_error||'');updateScaraTiming(scaraState.current,scaraState.percentage);return st;}catch(e){scaraState.error=String(e&&e.message||e);throw e;}}
...
async function startScaraItem(cfg,item){
 ...
 // Do not tell the ORYN UI that playback started until the controller confirms it.
 let last=null;
 for(let i=0;i<8;i++){
  last=await refreshScaraStatus(cfg);
  if(last.last_error)throw new Error(String(last.last_error));
  if(last.playing){scaraState.running=true;return true;}
  if(Number(last.progress||0)>=99.9){scaraState.running=false;return true;}
  await new Promise(r=>setTimeout(r,180));
 }
 throw new Error('ESP32 did not confirm pattern playback.');
}
async function startScaraSequence(seq){
 const cfg=scaraConfig();
 if(!cfg)throw new Error('ORYN SCARA is not active.');
 if(!Array.isArray(seq)||!seq.length)throw new Error('No SCARA pattern was supplied.');
 if(scaraWorker||scaraState.running)throw new Error('Another SCARA pattern is already running.');
 const before=await refreshScaraStatus(cfg);
 if(!before.referenced)throw new Error('SCARA reference required. Place both 76 mm arms straight at +X / 152 mm and press SCARA SET REFERENCE first.');
 scaraState.stopping=false;
 scaraState.error='';
 // Start and CONFIRM the first item before /run_theta_rho returns success to the UI.
 const first=seq[0];
 await startScaraItem(cfg,first);
 scaraWorker=(async()=>{
  try{
   await waitScaraPattern(cfg);
   for(const item of seq.slice(1)){
    if(scaraState.stopping)break;
    await startScaraItem(cfg,item);
    await waitScaraPattern(cfg);
   }
  }catch(e){
   scaraState.error=String(e&&e.message||e);
   mobileToast(scaraState.error);
  }finally{
   scaraState.running=false;
   scaraState.paused=false;
   scaraWorker=null;
  }
 })();
 return true;
...
async function scaraFetch(u,init={}){
 const cfg=scaraConfig();if(!cfg)return errResponse('ORYN SCARA table is not configured.',503);const p=u.pathname,m=String(init.method||'GET').toUpperCase(),b=parseBody(init);
 try{
  if(p==='/api/table-info'&&m==='GET')return jsonResponse({id:SCARA_ID,name:scaraDisplayName(),version:cfg.firmware||'V0.4'});
  if(p==='/api/table-info'&&m==='PATCH'){const name=String(b.name||'').trim();if(name){localStorage.setItem('oryn_scara_name',name);return jsonResponse({success:true,id:SCARA_ID,name});}return errResponse('Table name is required.',400);}
  if(p==='/api/settings'&&m==='GET'){const st=getLocalSettings();return jsonResponse({...st,app:{...(st.app||{}),name:scaraDisplayName()}});}
  if(p==='/api/settings'&&m==='PATCH')return jsonResponse(saveLocalSettings(b));
  if(p==='/serial_status') {const st=await refreshScaraStatus(cfg);return jsonResponse({connected:true,port:cfg.host,controller:'ORYN SCARA',firmware:'V0.4',referenced:!!st.referenced});}
  if(p==='/list_serial_ports')return jsonResponse([]);
  if(p==='/connect')return jsonResponse({success:true,connected:true,host:cfg.host});
  if(p==='/disconnect'){localStorage.setItem('oryn_scara_enabled','0');localStorage.setItem('orynmotion_active_table',OFFLINE_ID);ensureOfflineTable(false);return jsonResponse({success:true});}
  if(p==='/api/machine-hardware-profile')return jsonResponse({build:BUILD,read_only:true,controller_source:'oryn-scara-esp32',profile:{initialized:true,x:{driver:'28BYJ-48+ULN2003'},y:{driver:'28BYJ-48+ULN2003'}},geometry:{radius_mm:cfg.radius_mm,arm1_mm:cfg.arm1_mm,arm2_mm:cfg.arm2_mm,theta_calibrated:true,rho_calibrated:true,rho_travel_units:cfg.radius_mm,theta_revolution_units:360,reference_mode:'manual_physical_plus_x',auto_home_after_reference:scaraAutoHomeEnabled()}});
  if(p==='/api/pattern-designer/save'&&m==='POST'){if(!b||!b.thr)return errResponse('Missing generated THR.',400);const r=directBridgeJson('directSavePattern',String(b.name||'Pattern'),String(b.thr||''));return r&&r.success?jsonResponse(r):errResponse((r&&r.detail)||'Could not save generated pattern.',400);}
  if(p==='/delete_theta_rho_file'&&m==='POST'){const name=String(b.file_name||b.filename||b.path||''),x=patternEntry(name);if(!x||!x.native_path)return errResponse('Only locally generated Android patterns can be deleted here.',400);const ok=!!(window.OrynAndroid&&window.OrynAndroid.directDeletePattern&&window.OrynAndroid.directDeletePattern(name));return ok?jsonResponse({success:true}):errResponse('Could not delete local pattern.',500);}
  if(p==='/run_theta_rho'&&m==='POST'){const x=patternEntry(b.file_name);if(!x)return errResponse('Pattern is not available in the local ORYN library.',404);const seq=[];let clearName=null;if(b.pre_execution==='from_center'||b.pre_execution==='clear_from_in')clearName='clear_from_in.thr';else if(b.pre_execution==='from_perimeter'||b.pre_execution==='clear_from_out')clearName='clear_from_out.thr';else if(b.pre_execution==='adaptive'){try{const st=await refreshScaraStatus(cfg);clearName=Number(st.rho||0)>=.5?'clear_from_out.thr':'clear_from_in.thr';}catch(_){}}const c=clearName&&patternEntry(clearName);if(c)seq.push({path:c.path,display:c.path,rotate:false});seq.push({path:x.path,display:x.path,rotate:true});await startScaraSequence(seq);return jsonResponse({success:true,scara:true});}
  if(p==='/run_playlist'&&m==='POST'){const name=playlistRequestName(b),stored=normalizedPlaylistFiles(playlists()[name]||[]);if(!name||!stored.length)return errResponse('Playlist is empty or was not found.',404);const files=b.shuffle?[...stored].sort(()=>Math.random()-.5):stored,seq=[];for(const f of files){const x=patternEntry(f);if(x)seq.push({path:x.path,display:x.path,rotate:true});}if(!seq.length)return errResponse('No playable patterns were found.',404);await startScaraSequence(seq);return jsonResponse({success:true,scara:true,playlist_name:name,files:stored});}
  if(p==='/send_home'){await scaraHomeAction();return jsonResponse({success:true,scara:true,home:'center'});}
  if(p==='/move_to_center'||p==='/move_to_perimeter'){await scaraRequireReference(cfg);const rho=p==='/move_to_center'?0:1;await scaraJson(cfg.host,'/api/move_polar?theta_deg=0&rho='+rho,{method:'POST'});return jsonResponse({success:true,scara:true});}
  if(p==='/send_coordinate'){const theta=Number(b.theta||0),rho=Number(b.rho||0);await scaraJson(cfg.host,'/api/move_polar?theta_deg='+encodeURIComponent(theta*180/Math.PI)+'&rho='+encodeURIComponent(rho),{method:'POST'});return jsonResponse({success:true,scara:true});}
  if(p==='/set_speed'){const applied=await applyScaraSpeed(Number(b.speed)||scaraSavedSpeed());return jsonResponse({success:true,scara:true,speed:applied});}
  if(p==='/stop_execution'||p==='/force_stop'||p==='/soft_reset'){scaraState.stopping=true;await scaraJson(cfg.host,'/api/stop',{method:'POST'});scaraState.running=false;return jsonResponse({success:true});}
  if(p==='/pause_execution'){await scaraJson(cfg.host,'/api/pause',{method:'POST'});scaraState.paused=true;return jsonResponse({success:true});}
  if(p==='/resume_execution'){await scaraJson(cfg.host,'/api/resume',{method:'POST'});scaraState.paused=false;return jsonResponse({success:true});}
  if(p==='/api/wifi/status')return jsonResponse(await scaraJson(cfg.host,'/api/wifi/status'));
  if(p==='/api/wifi/networks'){const n=await scaraJson(cfg.host,'/api/wifi/scan');return jsonResponse(Array.isArray(n)?n:[]);}
  if(p==='/api/wifi/saved')return jsonResponse([]);
  ...
  if(p.startsWith('/api/rotation-calibration'))return jsonResponse({success:true,calibrated:true,theta_revolution_units:360,scara:true,read_only:true});
  if(p.startsWith('/api/perimeter-calibration'))return jsonResponse({success:true,calibrated:true,rho_travel_units:cfg.radius_mm,scara:true,read_only:true});
  return errResponse('This ORYN function is not mapped for SCARA V0.4 yet.',501);
 }catch(e){return errResponse(String(e&&e.message||e),503);}
}
function useScaraEndpoint(path){if(!scaraConfig())return false;if(path.startsWith('/api/rotation-calibration')||path.startsWith('/api/perimeter-calibration'))return true;return ['/api/table-info','/api/settings','/serial_status','/list_serial_ports','/connect','/disconnect','/run_theta_rho','/run_playlist','/send_home','/send_coordinate','/move_to_center','/move_to_perimeter','/set_speed','/stop_execution','/force_stop','/pause_execution','/resume_execution','/soft_reset','/api/machine-hardware-profile','/api/pattern-designer/save','/delete_theta_rho_file','/api/wifi/status','/api/wifi/networks','/api/wifi/saved'].includes(path);}
async function directFetch(u,init={}){
 ...
 if(p==='/run_theta_rho'&&m==='POST'){
  ...
  seq.push({asset:x.native_path||x.thr_url.replace(/^\//,''),display:x.path||String(b.file_name||'pattern.thr'),theta_offset_rad:readPatternOrientationDegrees()*Math.PI/180});
 ...
 if(p==='/run_playlist'&&m==='POST'){
  ...
  for(const file of files){
   ...
   const c=clearName&&patternEntry(clearName);if(c)seq.push({asset:c.native_path||c.thr_url.replace(/^\//,''),display:c.path||clearName});
   seq.push({asset:x.native_path||x.thr_url.replace(/^\//,''),display:x.path,theta_offset_rad:readPatternOrientationDegrees()*Math.PI/180});
  }
...
window.fetch=async function(input,init={}){
 ...
 if(u.hostname==='scara.oryn')return scaraFetch(u,init);
 ...
 if(useScaraEndpoint(p))return scaraFetch(new URL('http://scara.oryn'+u.pathname+u.search),init);
 ...
 if(p==='/api/settings'&&m==='GET'){const st=getLocalSettings();const appName=scaraConfig()?scaraDisplayName():(directConfig()?'ORYN Direct — ESP32 FluidNC':'ORYN Offline');return jsonResponse({...st,app:{...(st.app||{}),name:appName}});}
 ...
 if(p==='/api/machine-hardware-profile')return jsonResponse({build:'V7-Mobile',read_only:true,controller_source:'offline',supported_drivers:{A4988:[1,2,4,8,16],DRV8825:[1,2,4,8,16,32],TMC2208:[2,4,8,16],TMC2209:[1,2,4,8,16,32,64,128,256],TMC5160:[1,2,4,8,16,32,64,128,256],CUSTOM_STEP_DIR:[1,2,4,8,16,32,64,128,256]},profile:{initialized:false,x:{driver:'A4988',microsteps:16},y:{driver:'A4988',microsteps:16}},controller:{axes:{x:{},y:{}}},geometry:{theta_calibrated:false,theta_revolution_units:null,rho_calibrated:false,rho_travel_units:null}});
...
function scaraSocket(url){try{return new URL(String(url)).hostname==='scara.oryn';}catch(_){return false;}}
...
function mergeDiscovered(tables){
 try{
  ...
  const scaraActive=active===SCARA_ID&&localStorage.getItem('oryn_scara_enabled')==='1'&&validSavedScara(readScaraSavedRaw());
  const remoteMap=new Map();
  for(const t of old){if(t&&t.id!==OFFLINE_ID&&t.id!==DIRECT_ID&&t.id!==SCARA_ID&&t.url&&new URL(t.url,location.origin).origin!==location.origin)remoteMap.set(t.id,{...t,isCurrent:false});}
  for(const t of tables||[]){if(t&&t.id&&t.id!==OFFLINE_ID&&t.id!==DIRECT_ID&&t.id!==SCARA_ID&&t.url)remoteMap.set(t.id,{...t,name:orynBrandString(t.name||'ORYN'),isOnline:true,isCurrent:false});}
  const remotes=[...remoteMap.values()];
  const merged=directActive?[makeDirectTable(readDirectSavedRaw(),true),makeOfflineTable(false),...remotes]:scaraActive?[makeScaraTable(readScaraSavedRaw(),true),makeOfflineTable(false),...remotes]:[makeOfflineTable(true),...remotes];
  const validActive=active===OFFLINE_ID||(directActive&&active===DIRECT_ID)||(scaraActive&&active===SCARA_ID)||remotes.some(t=>t.id===active);
  const finalActive=validActive?active:OFFLINE_ID;
  localStorage.setItem('orynmotion_tables',JSON.stringify({tables:merged,activeTableId:finalActive}));localStorage.setItem('orynmotion_active_table',finalActive);
...
function installNativeButtons(){
 ...
 const scara=add.cloneNode(true);scara.id='oryn-native-scara-btn';scara.textContent='ORYN SCARA ESP32';scara.addEventListener('click',e=>{e.stopPropagation();openScaraConnect();});add.parentNode.insertBefore(scara,add);
 }
function closeScaraControlPanel(){const e=document.getElementById('oryn-scara-control-modal');if(e)e.remove();}
async function refreshScaraControlPanel(){const cfg=scaraConfig(),ref=document.getElementById('oryn-scara-refstate'),pos=document.getElementById('oryn-scara-pos'),auto=document.getElementById('oryn-scara-auto');if(!cfg||!ref)return;try{const st=await refreshScaraStatus(cfg);ref.textContent=st.referenced?'REFERENCED ✓':'REFERENCE REQUIRED';ref.style.color=st.referenced?'#98e0a4':'#f3bf7a';pos.textContent='Theta '+Number(st.theta_deg||0).toFixed(1)+'° · Rho '+Number(st.rho||0).toFixed(3);if(auto)auto.checked=scaraAutoHomeEnabled();}catch(e){ref.textContent='SCARA OFFLINE';ref.style.color='#ff9b9b';if(pos)pos.textContent=String(e&&e.message||e);}}
function openScaraControlPanel(){
 if(!scaraConfig()){mobileToast('Connect ORYN SCARA first.');return;}
 closeScaraControlPanel();const wrap=document.createElement('div');wrap.id='oryn-scara-control-modal';wrap.style.cssText='position:fixed;inset:0;z-index:2147483600;background:rgba(0,0,0,.82);display:flex;align-items:center;justify-content:center;padding:14px;font-family:system-ui,sans-serif';
 const card=document.createElement('div');card.style.cssText='width:min(680px,100%);max-height:92vh;overflow:auto;background:#101010;border:1px solid #3b3b3b;border-radius:20px;padding:16px;color:#eee;box-shadow:0 24px 80px #000';
 const cal=readScaraCalibrationCheck();
 card.innerHTML=`<div style="display:flex;justify-content:space-between;gap:10px;align-items:flex-start"><div><div style="font-size:20px;font-weight:850">ORYN SCARA Control</div><div style="font-size:12px;color:#aaa;margin-top:3px">76 + 76 mm · Full radius 152 mm · ESP32 V0.4</div></div><button id="oryn-sctl-close" style="border:0;background:#292929;color:#fff;border-radius:9px;padding:8px 11px">✕</button></div>
 <div style="margin-top:12px;padding:12px;border:1px solid #333;border-radius:12px;background:#171717"><div id="oryn-scara-refstate" style="font-weight:800">Checking…</div><div id="oryn-scara-pos" style="font-size:12px;color:#aaa;margin-top:4px">—</div></div>
 ...
 <div style="margin-top:12px;padding:11px;border:1px solid #333;border-radius:12px;background:#151515"><div style="display:flex;justify-content:space-between;gap:10px"><div><div style="font-weight:750">Drawing Speed</div><div style="font-size:11px;color:#aaa">10–100 mm/s · default 60 mm/s</div></div><div style="font-weight:850;color:#f2d377"><span id="oryn-scara-speed-val">60</span> mm/s</div></div><input id="oryn-scara-speed" type="range" min="10" max="100" step="5" value="60" style="width:100%;margin-top:10px"><button id="oryn-sctl-speed" class="oryn-sctl-main" style="width:100%;margin-top:8px">APPLY SPEED</button></div>
 ...
 document.getElementById('oryn-sctl-ref').onclick=()=>{if(!confirm('Place both 76 mm arms straight at +X with magnet centre exactly 152 mm from the main shaft. Set this as Theta 0° / Rho 1.00?'))return;run(scaraSetReferenceAction);};
 document.getElementById('oryn-sctl-home').onclick=()=>run(scaraHomeAction);document.getElementById('oryn-sctl-perim').onclick=()=>run(scaraPerimeterAction);document.getElementById('oryn-sctl-stop').onclick=()=>run(async()=>{const cfg=scaraConfig();await scaraJson(cfg.host,'/api/stop',{method:'POST'});scaraState.stopping=true;scaraState.running=false;mobileToast('SCARA stopped.');});
 const sld=document.getElementById('oryn-scara-speed'),sval=document.getElementById('oryn-scara-speed-val');if(sld){sld.value=String(scaraState.speed||scaraSavedSpeed());sval.textContent=String(Math.round(Number(sld.value)));sld.oninput=()=>{sval.textContent=String(Math.round(Number(sld.value)));};document.getElementById('oryn-sctl-speed').onclick=()=>run(async()=>{const applied=await applyScaraSpeed(Number(sld.value));sval.textContent=String(Math.round(applied));mobileToast('SCARA speed set to '+Math.round(applied)+' mm/s ✓');});}
 document.getElementById('oryn-scara-auto').checked=scaraAutoHomeEnabled();document.getElementById('oryn-scara-auto').onchange=e=>{setScaraAutoHomeEnabled(e.target.checked);mobileToast(e.target.checked?'SCARA Automatic Home enabled ✓':'SCARA Automatic Home disabled');};
 document.getElementById('oryn-sctl-radius').onclick=()=>run(scaraRadiusTest);document.getElementById('oryn-sctl-30').onclick=()=>run(scaraThirtyDegreeTest);document.getElementById('oryn-sctl-360').onclick=()=>{if(confirm('Run one complete 360° circle at Rho 0.95?'))run(scaraFullCircleTest);};document.getElementById('oryn-sctl-refresh').onclick=refreshScaraControlPanel;refreshScaraControlPanel();
}
function installScaraControlLauncher(){
 const existing=document.getElementById('oryn-scara-control-launcher');if(!scaraConfig()){if(existing)existing.remove();return;}if(existing)return;
 const b=document.createElement('button');b.id='oryn-scara-control-launcher';b.type='button';b.textContent='SCARA CONTROL';b.style.cssText='position:fixed;right:14px;top:70px;z-index:2147482500;border:1px solid #6d5a25;border-radius:999px;padding:9px 12px;background:#17140d;color:#f1c75b;font:800 11px system-ui;letter-spacing:.06em;box-shadow:0 6px 18px rgba(0,0,0,.35)';b.onclick=e=>{e.preventDefault();e.stopPropagation();openScaraControlPanel();};document.body.appendChild(b);
}
new MutationObserver(()=>{installNativeButtons();installPatternOrientationControl();installScaraControlLauncher();}).observe(document.documentElement,{childList:true,subtree:true});
...
sanitizeStoredOrynBranding();setTimeout(scrubVisibleLegacyBrand,250);setTimeout(installScaraControlLauncher,500);setTimeout(()=>{if(scaraConfig())applyScaraSpeed(scaraSavedSpeed()).catch(()=>{});},900);
...
setTimeout(()=>{try{
 ...
 if(window.OrynAndroid&&localStorage.getItem('oryn_direct_enabled')!=='1'&&localStorage.getItem('oryn_scara_enabled')!=='1')window.OrynAndroid.startDiscovery();
}catch(_){}},900);
})();
