(function(){
'use strict';
const BUILD='ORYN-DIRECT-ESP32-V10.4.1-DIRECT3.10-SYNCED-PLAYBACK-20260901-1';
...
function ensureOfflineTable(forceActive){
 try{
  ...
  if(forceActive||!old.some(t=>t&&t.id===active))active=OFFLINE_ID;
  let local=old.find(t=>t&&t.id===OFFLINE_ID);
  if(!local)local={id:OFFLINE_ID,name:'ORYN Offline',url:location.origin,isOnline:true,isCurrent:active===OFFLINE_ID,version:BUILD};
  else local={...local,name:'ORYN Offline',url:location.origin,isOnline:true,isCurrent:active===OFFLINE_ID,version:BUILD};
  const tables=[local,...old.filter(t=>t&&t.id!==OFFLINE_ID).map(t=>({...t,isCurrent:t.id===DIRECT_ID&&active===DIRECT_ID}))];
  if(!tables.some(t=>t.id===active))active=OFFLINE_ID;
  ...
  try{localStorage.setItem('orynmotion_active_table',OFFLINE_ID);}catch(__){}
 }
}
...
// DIRECT3.7: a real app cold start always opens ORYN in Offline mode.
...
function localKnownTables(){try{const d=JSON.parse(localStorage.getItem('orynmotion_tables')||'{}');return (d.tables||[]).filter(t=>t.id!==OFFLINE_ID&&!t.isCurrent).map(t=>({id:t.id,name:t.name,url:t.url,host:t.host,port:t.port,version:t.version}));}catch(_){return [];}}
...
function activateDirectTable(host,cfg){
 const thetaRev=Number(cfg&&cfg.thetaRev),rhoTravel=Number(cfg&&cfg.rhoTravel),rhoDirection=Number(cfg&&cfg.rhoDirection)<0?-1:1,feed=Number(cfg&&cfg.feed)||60;
 if(!(thetaRev>0)||!(rhoTravel>0)){smartWifiStatus('Enter valid Theta and Rho calibration values.',true);return false;}
 const saved={host,thetaRev,rhoTravel,rhoDirection,feed};localStorage.setItem('oryn_direct_last',JSON.stringify(saved));localStorage.setItem('oryn_direct_enabled','1');
 try{
  const raw=JSON.parse(localStorage.getItem('orynmotion_tables')||'{}'),tables=Array.isArray(raw.tables)?raw.tables:[];
  const t={id:DIRECT_ID,name:'ORYN Direct — ESP32 FluidNC',url:location.origin,host,directHost:host,directFluidNC:true,thetaRevUnits:thetaRev,rhoTravelUnits:rhoTravel,rhoDirection,directFeed:feed,isOnline:true,isCurrent:true,version:BUILD};
  const merged=[...tables.filter(x=>x&&x.id!==DIRECT_ID).map(x=>({...x,isCurrent:false})),t];
  ...
  smartWifiStatus('Connected ✓  ORYN Direct table is active.');
  setTimeout(()=>location.reload(),550);return true;
...
function probeAndActivateDirect(host){
 const p=directBridgeJson('directProbe',host);if(!p||!p.ok){smartWifiStatus('Wi‑Fi joined, but FluidNC did not answer at '+host+'. Keep the ESP32 powered and try again.',true);return false;}
 const stableHost=String((p&&p.host)||host||'').trim()||host;
 const old=readSavedDirectCalibration();
 if(hasSavedDirectCalibration(old)){smartWifiStatus('FluidNC detected at '+stableHost+' ✓  Activating ORYN Direct with your saved calibration…');return activateDirectTable(stableHost,old);}
...
function openSmartWifi(){
 ...
 <div style="display:grid;grid-template-columns:1fr 1fr;gap:9px"><button id="oryn-sw-scan" class="oryn-sw-btn">Scan Wi‑Fi</button><button id="oryn-sw-lan" class="oryn-sw-btn">Find ESP32 on Home Wi‑Fi</button></div>
 ...
 <details style="margin-top:16px;border-top:1px solid #292929;padding-top:12px"><summary style="cursor:pointer;font-weight:700">Wi‑Fi network setup (change anytime)</summary><div style="font-size:12px;color:#aaa;margin:8px 0">At a new place, if the saved network is unavailable the FluidNC AP returns as fallback. Connect to FluidNC, enter the new router or phone-hotspot SSID/password here, save it, then after reboot use Find ESP32 on Home Wi‑Fi.</div><input id="oryn-home-ssid" placeholder="Wi‑Fi network name (SSID)" style="width:100%;box-sizing:border-box;margin:5px 0;padding:10px;border-radius:9px;border:1px solid #333;background:#181818;color:#fff"><input id="oryn-home-pass" type="password" placeholder="Wi‑Fi network password" style="width:100%;box-sizing:border-box;margin:5px 0;padding:10px;border-radius:9px;border:1px solid #333;background:#181818;color:#fff"><button id="oryn-home-save" class="oryn-sw-btn" style="width:100%;margin-top:6px">Save Wi‑Fi Network to ESP32</button></details>
 ...
 document.getElementById('oryn-sw-lan').onclick=()=>{smartWifiStatus('Searching current Internet Wi‑Fi for FluidNC…');try{window.OrynAndroid.discoverFluidNcLan();}catch(e){smartWifiStatus(String(e),true);}};
 document.getElementById('oryn-home-save').onclick=()=>{const ssid=document.getElementById('oryn-home-ssid').value.trim(),pass=document.getElementById('oryn-home-pass').value;if(!ssid){smartWifiStatus('Enter the Wi‑Fi network name.',true);return;}smartWifiStatus('Writing Wi‑Fi network settings to FluidNC…');const r=directBridgeJson('configureFluidNcHomeWifi',ssid,pass);if(r&&r.success)smartWifiStatus(r.message||'Saved. Power-cycle ESP32, connect this phone to that Wi‑Fi, then use Find ESP32 on Home Wi‑Fi. You can change this network again later without reflashing.');else smartWifiStatus((r&&r.detail)||'Could not save Wi‑Fi network.',true);};
...
function rehydrateDirectTransport(){
 ...
 setTimeout(()=>{try{
  ...
  // DIRECT3.6: Home/pattern playback owns the FluidNC Telnet session. Never
  // open a background $I probe while motion is active.
  if(st.is_running||st.is_homing)return;
  const q=directBridgeJson('directProbe',cfg.host);
...
// DIRECT3.6 diagnostic only: surface the exact native motion failure once.
...
async function directFetch(u,init={}){
 ...
 if(p==='/api/table-info')return jsonResponse({id:DIRECT_ID,name:'ORYN Direct — ESP32 FluidNC',version:BUILD});
 ...
 if(p==='/move_to_center'){if(directCalibrationActive())return errResponse('Finish or save the active machine calibration first.',409);const r=directBridgeJson('directAction',cfg.host,`G90\nG1 Y0 F${cfg.feed.toFixed(3)}`);return jsonResponse({success:!!(r&&r.success)});}
 if(p==='/move_to_perimeter'){if(directCalibrationActive())return errResponse('Finish or save the active machine calibration first.',409);const y=cfg.rhoTravel*cfg.rhoDirection;const r=directBridgeJson('directAction',cfg.host,`G90\nG1 Y${y.toFixed(5)} F${cfg.feed.toFixed(3)}`);return jsonResponse({success:!!(r&&r.success)});}
 if(p==='/send_coordinate'){const x=(Number(b.theta||0)/(Math.PI*2))*cfg.thetaRev,y=Number(b.rho||0)*cfg.rhoTravel*cfg.rhoDirection;const r=directBridgeJson('directAction',cfg.host,`G90\nG1 X${x.toFixed(5)} Y${y.toFixed(5)} F${cfg.feed.toFixed(3)}`);return jsonResponse({success:!!(r&&r.success)});}
...
function mergeDiscovered(tables){
 try{
  const raw=JSON.parse(localStorage.getItem('orynmotion_tables')||'{}'),old=raw.tables||[];
  let local=old.find(t=>t.id===OFFLINE_ID);if(!local)local={id:OFFLINE_ID,name:'ORYN Offline',url:location.origin,isOnline:true,isCurrent:true,version:BUILD};
  const byId=new Map([[local.id,local]]);for(const t of old){if(t.id!==OFFLINE_ID)byId.set(t.id,{...t,isCurrent:false});}for(const t of tables||[]){if(t&&t.id)byId.set(t.id,{...t,name:orynBrandString(t.name||'ORYN'),isOnline:true,isCurrent:false});}
  const merged=[...byId.values()];let active=localStorage.getItem('orynmotion_active_table')||OFFLINE_ID;
  if(!merged.some(t=>t.id===active))active=OFFLINE_ID;
  const prevSig=JSON.stringify(old.map(x=>[x.id,x.url]).sort());const nextSig=JSON.stringify(merged.map(x=>[x.id,x.url]).sort());
  localStorage.setItem('orynmotion_tables',JSON.stringify({tables:merged,activeTableId:active}));
  localStorage.setItem('orynmotion_active_table',active);
  // DIRECT3.6: discovery must never reload the running app. A reload used to
  // race React's table refresh, causing a repeating unstyled flash and could
  // interrupt a live Direct FluidNC pattern session. The table picker will see
  // discovered entries on the next deliberate navigation/restart instead.
  void prevSig; void nextSig;
 }catch(e){console.error('ORYN native discovery merge failed',e);}
}
window.__orynNativeDiscovery=function(tables){mergeDiscovered(Array.isArray(tables)?tables:[]);const b=document.getElementById('oryn-native-scan-btn');if(b)b.textContent=(tables&&tables.length)?'ORYN found':'Scan for ORYN';if(!tables||!tables.length)mobileToast('No ORYN table found on this Wi‑Fi. You can add its IP manually.');};
...
function installNativeButtons(){
 ...
 if(!add||document.getElementById('oryn-native-scan-btn'))return;
...
// DIRECT3.10: keep the visible machine identity synchronized with the
// authoritative active-table storage even if the React shell still holds its
// pre-connect title in memory. This never opens a controller socket.
function syncDirectVisibleIdentity(){
 try{
  const active=localStorage.getItem('orynmotion_active_table');
  const enabled=localStorage.getItem('oryn_direct_enabled')==='1';
  const isDirect=active===DIRECT_ID&&enabled&&!!directConfig();
  const wanted=isDirect?'ORYN Direct — ESP32 FluidNC':'ORYN Offline';
  if(isDirect)document.title=wanted;
  const walker=document.createTreeWalker(document.body||document.documentElement,NodeFilter.SHOW_TEXT);
  let n;
  while((n=walker.nextNode())){
   const v=(n.nodeValue||'').trim();
   if(isDirect&&v==='ORYN Offline'){n.nodeValue=n.nodeValue.replace('ORYN Offline',wanted);break;}
   if(!isDirect&&active===OFFLINE_ID&&v==='ORYN Direct — ESP32 FluidNC'){n.nodeValue=n.nodeValue.replace('ORYN Direct — ESP32 FluidNC','ORYN Offline');break;}
  }
 }catch(_){}
}
new MutationObserver(syncDirectVisibleIdentity).observe(document.documentElement,{childList:true,subtree:true,characterData:true});
setInterval(syncDirectVisibleIdentity,750);
setTimeout(syncDirectVisibleIdentity,50);
ensureOfflineTable(false);
...
setTimeout(()=>{try{
 // DIRECT3.6: once Direct ESP32 is configured, do not launch the unrelated
