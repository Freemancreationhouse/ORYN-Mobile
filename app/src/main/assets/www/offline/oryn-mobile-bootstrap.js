...
async function scaraFetch(u,init={}){
 ...
 try{
  ...
  if(p==='/set_speed')return jsonResponse({success:true,scara:true,note:'SCARA V0.4 uses firmware-configured drawing speed.'});
...
sanitizeStoredOrynBranding();setTimeout(scrubVisibleLegacyBrand,250);setTimeout(installScaraControlLauncher,500);setTimeout(()=>maybeScaraAutoHome('connect'),1400);
setTimeout(()=>{try{
