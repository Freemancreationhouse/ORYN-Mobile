...
const isORYNMobile=!!window.OrynAndroid || integrationParams.get('mobile')==='1';
if(isORYNMobile) document.body.classList.add('oryn-mobile');
...
function saveBlob(name,data,type='text/plain'){
  if(window.OrynAndroid&&typeof window.OrynAndroid.saveFile==='function'){
    try{window.OrynAndroid.saveFile(name,type,String(data),false);setStatus('SAVE FILE','busy');return;}catch(err){console.warn('ORYN native save unavailable',err);}
  }
...
function syncLibraryTarget(){
  ...
  }else saveLibraryBtn.disabled=false;
}
async function saveToORYNLibrary(){
  ...
  }finally{if(isORYNMobile)setTimeout(syncLibraryTarget,1200);else saveLibraryBtn.disabled=false;}
}
...
$('#downloadPng').onclick=()=>{draw();const name=safeName()+'.png',url=canvas.toDataURL('image/png');if(window.OrynAndroid&&typeof window.OrynAndroid.saveFile==='function'){try{window.OrynAndroid.saveFile(name,'image/png',url.split(',')[1]||'',true);setStatus('SAVE FILE','busy');return;}catch(err){console.warn('ORYN native PNG save unavailable',err);}}const a=document.createElement('a');a.download=name;a.href=url;a.click();};

window.__orynPatternDesignerFileSaved=function(ok,name,message){setStatus(ok?'READY':'ERROR',ok?'ready':'error');if(message)setSaveMessage(message,ok?'ok':'error');};
