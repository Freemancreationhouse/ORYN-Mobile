...
- Settings → **Open WiFi Setup** is now mode-aware: Offline/Direct opens Android Smart Wi-Fi, while an active ORYN Pi opens its real `/wifi-setup` page and remote API.
...
- The compiled Android router retains the Pi Wi-Fi component for Pi mode without exposing it to Offline or Direct modes, eliminating the black-screen path.
- Direct pattern playback now has a saved **Pattern orientation** selector: 0°, 90°, 180°, 270° or custom degrees.
- Orientation rotates the selected physical pattern without modifying its library `.thr` file; clearing patterns are not rotated.
